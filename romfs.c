...
static void romfs_free_dir_list(romfs_dirent_ctx_t *root_ctx)
{
    while (root_ctx != NULL)
    {
        romfs_dirent_ctx_t *next = root_ctx->next;
        free(root_ctx);
        root_ctx = next;
    }
}

static void romfs_free_file_list(romfs_fent_ctx_t *files)
{
    while (files != NULL)
    {
        romfs_fent_ctx_t *next = files->next;
        free(files);
        files = next;
    }
}
